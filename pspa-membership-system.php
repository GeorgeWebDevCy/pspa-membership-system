...
/**
 * Redirect add-to-cart requests from the front page straight to checkout.
 *
 * The membership purchase button links directly to `/?add-to-cart=...`. WooCommerce
 * handles the request but then redirects back to the same URL, so the homepage
 * loads instead of the checkout. By swapping the redirect target we send members
 * straight to the checkout with their cart intact.
 *
 * @param string $redirect_url Redirect destination generated by WooCommerce.
 * @param \WC_Product|false $product Product being added to the cart, or false on failure.
 * @return string
 */
function pspa_ms_redirect_home_add_to_cart_to_checkout( $redirect_url, $product ) {
    if ( ( is_admin() && ! wp_doing_ajax() ) || ! isset( $_REQUEST['add-to-cart'] ) ) {
        return $redirect_url;
    }
    ...
    $home_path = $home_path ? rawurldecode( untrailingslashit( $home_path ) ) : '';
...
add_filter( 'woocommerce_add_to_cart_redirect', 'pspa_ms_redirect_home_add_to_cart_to_checkout', 10, 2 );
